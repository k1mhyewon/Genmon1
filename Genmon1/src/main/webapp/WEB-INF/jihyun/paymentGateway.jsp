...
$(document).ready(function() {
   ...
   IMP.init('imp47370456');  // 중요!!!!!  아임포트에 가입시 부여받은 "가맹점 식별코드". 
   ...
   IMP.request_pay({
       ...
       amount : '100',	  // ${purchaseMoney}  결제 금액 number 타입. 필수항목. 
       buyer_email : '${buyer_email}',  // 구매자 email
       buyer_name : '${buyer_name}',	  // 구매자 이름 
       buyer_tel : '${buyer_tel}',    // 구매자 전화번호 (필수항목)
       buyer_addr : '${buyer_addr}',  
       buyer_postcode : '${buyer_postcode}',
       m_redirect_url : '<%=ctxPath%>/order/purchaseEnd.sun'  // 휴대폰 사용시 결제 완료 후 action : 컨트롤러로 보내서 자체 db에 입력시킬것!
   }, function(rsp) {
       /*
		   if ( rsp.success ) {
			   var msg = '결제가 완료되었습니다.';
	   ...
	   */
		...
		*/
		...
		//  $(opener.location).attr("href", "javascript:goCoinUpdate('${userid}','${coinmoney}');");
			...
			window.opener.goPurchaseEnd();
		    self.close();
			
        } else {
        	location.href="<%=ctxPath%>/index.sun";
            alert("결제에 실패하였습니다.");
       }
