...
$(document).ready(function() {
   ...
   IMP.init('imp11533287');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드". imp41442787
	
   // 결제요청하기
   IMP.request_pay({
       pg : 'html5_inicis', // 결제방식 PG사 구분
       pay_method : 'card',	// 결제 수단
       merchant_uid : 'merchant_kimminsu' + new Date().getTime(), // 가맹점에서 생성/관리하는 고유 주문번호
       name : '${requestScope.productName}'+ " 결제",   // 결제테스트(코인충전|주문명) 코인충전 또는 order 테이블에 들어갈 주문명 혹은 주문 번호. (선택항목)원활한 결제정보 확인을 위해 입력 권장(PG사 마다 차이가 있지만) 16자 이내로 작성하기를 권장
       amount :	100,							       // ${requestScope.coinmoney}  결제 금액 number 타입이므로 ''쓰지않는다. 필수항목. 
	   ...
	   */
		...
		if ( rsp.success ) { // PC 데스크탑용. 결제 성공한경우 
